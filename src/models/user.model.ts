...
export interface IUserState {
  ...
  s3Credentials: { accessKey: string; secretKey: string };
}
...
export default class User extends Model {
  ...
  static async checkLoginStatus() {
    try {
      ...
      const response = await axios.get(
        `${this.hydroshareHost}/hsapi/userInfo/`,
        {
  ...
  static async getResourceS3prefix(
    ...
    try {
      const response = await axios.get(
        `${this.hydroshareHost}/hsapi/resource/s3/${res_id}/`,
        {
  ...
  static async getOrCreateS3Credentials() {
    ...
    if (
      this.$state.s3Credentials.accessKey &&
      this.$state.s3Credentials.secretKey
    ) {
      return this.$state.s3Credentials;
    }
    try {
      ...
      const response = await axios.post(
        `${this.hydroshareHost}/hsapi/user/service/accounts/s3/`,
        {},
        {
          withCredentials: true, // Ensure cookies are sent for authentication
          headers: headers,
      ...
      if (
        ...
        try {
          ...
          if (retryResponse.status === 200 && retryResponse.data) {
            console.log(
              "getOrCreateS3Credentials retry response:",
              retryResponse.data,
            );
            ...
            await User.commit((state) => {
              state.s3Credentials = {
                accessKey: retryResponse.data.access_key,
                secretKey: retryResponse.data.secret_key,
              };
            });
          ...
          throw new Error(
            `Failed to create S3 credentials after retry: ${retryError.message}`,
          );
        }
      }
  ...
  static async manageResourceAccess(
    resource_id: string,
    user_id: number,
    privilege: number | PrivilegeCodes,
  ) {
    ...
    try {
      ...
      if (privilege === PrivilegeCodes.NONE) {
        ...
        console.log(
          "manageResourceAccess DELETE response:",
          response.status,
          response.data,
        );
      } else {
        ...
        console.log(
          "manageResourceAccess PUT response:",
          response.status,
          response.data,
        );
      }
      ...
      if (
        ...
        try {
          ...
          if (privilege === PrivilegeCodes.NONE) {
            ...
            retryResponse = await axios.delete(
          ...
          if (retryResponse.status >= 200 && retryResponse.status < 300) {
            console.log(
              "manageResourceAccess retry response:",
          ...
          throw new Error(
            `Failed to manage resource access after retry: ${retryError.message}`,
          );
        }
      }
